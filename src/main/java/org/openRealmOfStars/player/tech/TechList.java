...
public class TechList {
  ...
  /**
   * Maximum number of tech levels (default, can be overridden)
   */
  ...
  /**
   * Maximum tech level for this tech list
   */
   ...
   * @param race SpaceRace for correct tech tree
   * @param maxTechLevel Maximum tech level (minimum 10)
   */
  public TechList(final SpaceRace race, final int maxTechLevel) {
    this.race = race;
    this.maxTechLevel = Math.max(maxTechLevel, MAX_TECH_LEVEL);
    techList = new TechListForLevel[TechType.values().length][this.maxTechLevel];
    for (int i = 0; i < MAX_TECH_TYPES; i++) {
      for (int j = 0; j < this.maxTechLevel; j++) {
